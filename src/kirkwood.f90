...
do n=0,nt
	...
	if (n==0) then
		Bcf_pbe=1/rds*(1/((1+cappa*rds)*eps1)-1/eps0)
	else
		if (cappa<1.d-20) then 
			xx=n+1
		else
	        xx=(n+1)+((cappa*rds)**2*fkn(cappa*rds,n-1))/(2*n-1)/fkn(cappa*rds,n)
		endif
	...
	Ccf_pbe=exp(cappa*rds)*(1/eps0+rds**(2*n+1)*Bcf_pbe)/fkn(cappa*rds,n)
	...
	do m=-n,n
		...
		if (iside<=0) then
			...
			cc=Cnm(1)*exp(-cappa*r)*fkn(cappa*r,n)/r**(n+1)
			dd=Cnm(2)*exp(-cappa*r)*fkn(cappa*r,n)/r**(n+1)
		endif
		ptltp1=aa*cc-bb*dd
		pmttp1=aa*ee-bb*ff
		psvtp1=aa*gg-bb*hh
...
temp1=1.d0
do m=1,n
    temp1=temp1/dble(n+m)
enddo
!print *,'temp=',temp

do is=0,n
!	fkn=fkn+x**is*(2**is)*fact(n)*fact(2*n-is)/(fact(is)*fact(2*n)*fact(n-is))
	fkn=fkn+temp1*x**is*(2**is)*fact(2*n-is)/(fact(is)*fact(n-is))
        !print *,is,fkn
enddo
1022 continue
end
...
IMPLICIT DOUBLE PRECISION (a-h,o-z)
DIMENSION PM(0:200,0:200),PD(0:200,0:200), flp(-nn:nn,0:nn)
CALL LPMN(200,mm,nn+1,xx,PM,PD)
flp=0.d0
DO J=0,nn
    do m=0,j
		flp(m,j)=pm(m,j)
