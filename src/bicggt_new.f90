...
if (ijka(1).ne.n+2) write(*,*) 'mismatched vector and matrix in sprsax'
do i=1,n
...
INTEGER*4 i,j,k
if (ijka(1).ne.n+2) write(*,*) 'mismatched vector and matrix in sprstx'
	...
	do j=1,n							!Loop over columns.
		if(dabs(a(i,j)).ge.thresh)then
			if(i.ne.j)then				!Store off-diagonal elements and their columns.
				k=k+1
				if(k.gt.nmax) write(*,*) 'nmax too small in sprsin'
				sa(k)=a(i,j)
				ijka(k)=j
			endif
		endif
